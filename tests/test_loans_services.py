...
def test_direct_booking_active_when_starts_today():
    ...
    user = UserFactory()
    booking = create_booking(item=item, user=user, start_date=_today(), end_date=_today() + timedelta(days=2))
...
def test_direct_booking_reserved_when_starts_later():
    ...
    booking = create_booking(item=item, user=user, start_date=start, end_date=start + timedelta(days=1))
...
def test_requires_approval_creates_request():
    ...
    user = UserFactory()
    booking = create_booking(item=item, user=user, start_date=_today(), end_date=_today())
...
def test_responsible_books_directly_despite_approval_policy():
    ...
    booking = create_booking(item=item, user=responsible, start_date=_today(), end_date=_today())
    assert booking.status == Booking.Status.ACTIVE
...
def test_request_for_occupied_period_is_rejected():
    """Eine Anfrage für einen belegten Zeitraum könnte nie genehmigt werden und wird gleich abgelehnt."""
    item = ItemFactory(loan_policy=Item.LoanPolicy.APPROVAL)
    BookingFactory(
        item=item,
...
def test_defect_item_cannot_be_booked():
    ...
    with pytest.raises(BookingError, match="Defekt"):
        create_booking(item=item, user=user, start_date=_today(), end_date=_today())
...
def test_start_in_past_rejected():
    ...
    user = UserFactory()
    with pytest.raises(BookingError):
        create_booking(item=item, user=user, start_date=_today() - timedelta(days=1), end_date=_today())
...
def test_overdue_active_booking_still_blocks():
    ...
    other = UserFactory()
    with pytest.raises(BookingError):
        create_booking(item=item, user=other, start_date=_today(), end_date=_today() + timedelta(days=1))
...
def test_email_sent_on_request(django_capture_on_commit_callbacks, mailoutbox):
    ...
    with django_capture_on_commit_callbacks(execute=True):
        create_booking(item=item, user=borrower, start_date=_today(), end_date=_today())

...
def test_events_feed_filters_by_item(client, user):
    ...
    booking_a = BookingFactory(
        item=item_a, status=Booking.Status.RESERVED, start_date=_today(), end_date=_today()
    )
...
def test_events_feed_filters_by_borrower(client, user):
    mine = BookingFactory(
        borrower=user, status=Booking.Status.RESERVED, start_date=_today(), end_date=_today()
    )
...
def test_booking_create_direct_booking_triggers_htmx_event(
    client, user, django_capture_on_commit_callbacks, mailoutbox
):
